...
import { Languages, Loader2, UserRound, Globe, PenLine, FileText } from "lucide-react";
import type { ReviewTranslations, TranslatableLocale } from "@/lib/types";
...
export default function ReviewTranslationsEditor({
  formRef,
  initialTranslations,
}: {
  formRef: React.RefObject<HTMLFormElement | null>;
  initialTranslations: ReviewTranslations;
}) {
  const [translations, setTranslations] = useState<ReviewTranslations>(initialTranslations || {});
  ...
  function patchCurrent(patch: Partial<NonNullable<ReviewTranslations[TranslatableLocale]>>) {
  ...
  async function handleSuggest() {
    ...
    try {
      ...
      const fields = {
        ...
        customer_country: ((fd?.get("customer_country") as string) || "").trim(),
        title: ((fd?.get("title") as string) || "").trim(),
        quote: ((fd?.get("quote") as string) || "").trim(),
      };
      const result = await suggestTranslations({ targetLocale: active, fields });
      patchCurrent({
        customer_name: result.customer_name || current.customer_name,
        customer_country: result.customer_country || current.customer_country,
        title: result.title || current.title,
        quote: result.quote || current.quote,
      });
    } catch {
  ...
  return (
    <AdminSection
      ...
      icon={Languages}
      description="ทับศัพท์ชื่อ/ประเทศลูกค้าได้ที่นี่ (เผื่อชื่อเป็นภาษาไทยอ่านไม่ออกสำหรับลูกค้าต่างชาติ) รวมถึงหัวข้อ/ข้อความรีวิว — ช่องไหนไม่กรอกจะโชว์ข้อความจากฟอร์มด้านบนแทนอัตโนมัติ ใส่คำแปลภาษาไทยที่นี่ได้ด้วยถ้าฟอร์มด้านบนพิมพ์เป็นภาษาอื่นไว้"
    >
      ...
      <div className="mb-3 grid gap-3 sm:grid-cols-2">
        <label className="block">
          <span className="mb-1 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[var(--color-gold)]">
            <UserRound size={13} /> ชื่อลูกค้า (ทับศัพท์)
          </span>
          <input value={current.customer_name || ""} onChange={(e) => patchCurrent({ customer_name: e.target.value })} className="t-input" />
        </label>
        <label className="block">
          <span className="mb-1 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[var(--color-gold)]">
            <Globe size={13} /> ประเทศ (ทับศัพท์)
          </span>
          <input value={current.customer_country || ""} onChange={(e) => patchCurrent({ customer_country: e.target.value })} className="t-input" />
        </label>
      </div>
      <label className="mb-3 block">
        <span className="mb-1 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[var(--color-gold)]">
          <PenLine size={13} /> หัวข้อรีวิว
        </span>
        <input value={current.title || ""} onChange={(e) => patchCurrent({ title: e.target.value })} className="t-input" />
      </label>
      <label className="block">
        <span className="mb-1 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[var(--color-gold)]">
          <FileText size={13} /> ข้อความรีวิว
        </span>
        <textarea rows={3} value={current.quote || ""} onChange={(e) => patchCurrent({ quote: e.target.value })} className="t-input resize-none" />
      </label>
